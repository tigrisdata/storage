...
describe('computeCompletions', () => {
  describe('command completion', () => {
    ...
    it('filters built-in commands by prefix', async () => {
      const [hits, token] = await computeCompletions('gr', {
        shell: null,
        cwd: undefined,
      });
    ...
    it('filters REPL commands by prefix', async () => {
      const [hits, token] = await computeCompletions('mou', {
        shell: null,
        cwd: undefined,
      });
    ...
    it('filters custom commands by prefix', async () => {
      const [hits, token] = await computeCompletions('pres', {
        shell: null,
        cwd: undefined,
      });
    ...
    it('returns full list when no command matches prefix', async () => {
      const [hits] = await computeCompletions('zzzzzz', {
        shell: null,
        cwd: undefined,
      });
  ...
  describe('bucket-name argument completion', () => {
    ...
    it("completes bucket names for 'fork'", async () => {
      ...
      const [forkHits] = await computeCompletions('fork ', {
        shell,
        cwd: undefined,
      });
  ...
  describe('mount-point argument completion', () => {
    ...
    it("completes mount points for 'flush'", async () => {
      ...
      const [hits] = await computeCompletions('flush ', {
        shell,
        cwd: undefined,
      });
  ...
  describe('path completion', () => {
    it('lists cwd entries for bare token', async () => {
      ...
      await shell.exec(
        'mkdir -p /workspace && touch /workspace/notes.md /workspace/data.json'
      );
    ...
    it('returns empty for path completion when shell is not configured', async () => {
      const [hits] = await computeCompletions('ls foo', {
        shell: null,
        cwd: undefined,
      });
      expect(hits).toEqual([]);
  ...
  describe('via ReplSession default cwd', () => {
    it('falls back to engine cwd when ctx.cwd is undefined', async () => {
      ...
      const [, token] = await computeCompletions('ls fo', {
        shell,
        cwd: undefined,
      });
