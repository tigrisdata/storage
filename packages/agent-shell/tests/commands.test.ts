...
describe('presign', () => {
  ...
  describe('OAuth session (no access key in config)', () => {
    ...
    it('succeeds when --key is provided', async () => {
      ...
      const result = await oauthCmd.execute(
        ['/file.txt', '--key', 'tid_user'],
        makeCtx()
      );
      expect(result.exitCode).toBe(0);
      expect(vi.mocked(getPresignedUrl)).toHaveBeenCalledWith(
...
describe('snapshot', () => {
  ...
  it('falls back to cwd bucket when positional omitted', async () => {
    const cmdWithResolve = createSnapshotCommand(config, {
      resolveBucket: stubResolveBucket,
    });
        ...
        snapshots: [
          {
            version: '1713200000',
            name: 'v1',
            creationDate: new Date('2026-04-15T00:00:00Z'),
          },
          {
            version: '1713300000',
            creationDate: new Date('2026-04-16T00:00:00Z'),
          },
        ],
      },
    });
...
describe('fork', () => {
  ...
  it('creates a fork from a snapshot', async () => {
    ...
    await cmd.execute(
      ['source', '--name', 'fork-name', '--snapshot', '1713200000'],
      makeCtx()
    );
  ...
  it('falls back to cwd bucket for source when omitted', async () => {
    const cmdWithResolve = createForkCommand(config, {
      resolveBucket: stubResolveBucket,
    });
    vi.mocked(createBucket).mockResolvedValue({ data: {} as never });

    await cmdWithResolve.execute(
      ['--name', 'new-fork'],
      makeCtx('/my-bucket/sub')
    );
  ...
  describe('--list', () => {
    ...
    it('falls back to cwd bucket when source omitted', async () => {
      ...
      const result = await cmdWithResolve.execute(
        ['--list'],
        makeCtx('/my-bucket')
      );
      expect(result.exitCode).toBe(0);
      expect(vi.mocked(listForks)).toHaveBeenCalledWith(
        'my-bucket',
        expect.anything()
      );
    });
    ...
    it('rejects --snapshot + --list', async () => {
      ...
      expect(result.stderr).toContain(
        '--snapshot and --list cannot be combined'
      );
    });
