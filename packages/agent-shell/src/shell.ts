...
export class TigrisShell {
  ...
  mount(bucket: string, mountPoint: string): void {
    const normalized = mountPoint.startsWith('/')
      ? mountPoint
      : `/${mountPoint}`;
  ...
  async flush(mountPoint?: string): Promise<void> {
        ...
        m.adapter.flush().catch((err: unknown) => {
          errors.push(
            err instanceof Error
              ? err
              : new Error(`flush ${m.mountPoint}: ${String(err)}`)
          );
        })
      )
    );
    ...
    if (errors.length > 0) {
      throw new AggregateError(
        errors,
        `flush failed: ${errors.length} mount(s) failed`
      );
    }
  }

  /** Resolve an absolute path to its bucket and object key. */
  private resolveBucketForPath(
    absolutePath: string
    ...
    for (const m of this.mounts) {
      if (
        ...
        normalized.startsWith(`${m.mountPoint}/`)
      ) {
