...
export class TigrisAdapter implements IFileSystem {
  ...
  async readFile(
    ...
    const result = await get(this.toKey(normalized), 'string', {
    ...
    const headResult = await head(this.toKey(normalized), {
      config: this.sdkConfig,
    });
    const mtime =
      headResult && 'data' in headResult && headResult.data
        ? headResult.data.modified
        : new Date();
  ...
  async exists(path: string): Promise<boolean> {
    ...
    try {
      const result = await head(this.toKey(normalized), {
        config: this.sdkConfig,
      });
      ...
      return (
        ...
        listResult.data.commonPrefixes.length > 0
      );
    } catch {
      return false;
    }
  ...
  private async collectRemoteEntries(
    normalized: string,
    entries: Set<string>
  ): Promise<void> {
    try {
  ...
  async flush(): Promise<void> {
    ...
    await Promise.all(
      dirty.map(({ path, entry }) => {
        ...
        return put(this.toKey(path), body, { config: this.sdkConfig }).then(
          (result) => {
            if ('error' in result) {
              errors.push(
                new Error(`flush put "${path}": ${result.error.message}`)
              );
            }
          }
        );
    ...
    await Promise.all(
      deleted.map((path) =>
        remove(this.toKey(path), { config: this.sdkConfig }).then((result) => {
          if ('error' in result) {
            errors.push(
              new Error(`flush remove "${path}": ${result.error.message}`)
            );
          }
        })
      )
    ...
    if (errors.length > 0) {
      throw new AggregateError(
        errors,
        `flush failed: ${errors.length} operation(s) failed`
      );
    }
