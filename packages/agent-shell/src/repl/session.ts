...
export class ReplSession {
  ...
  private async listAndMountBuckets(
    ...
    io.write(
      `Mounted ${bucketNames.length} bucket(s) at /. Run 'df' to list them.\n`
    );
  }
    ...
    try {
      ...
      if (result.organizations.length > 1) {
        ...
        if (
          Number.isNaN(index) ||
          index < 0 ||
          index >= result.organizations.length
        ) {
  ...
  private handleDf(io: ReplIO): void {
    ...
    const col =
      Math.max('Bucket'.length, ...mounts.map((m) => m.bucket.length)) + 2;
  ...
  private handleHelp(io: ReplIO): void {
    io.write('Session:\n');
    io.write(
      '  login                                         Login with Tigris (OAuth)\n'
    );
    io.write(
      '  configure --key <id> --secret <key> [--bucket <name>] [--endpoint <url>]\n'
    );
    io.write(
    ...
    io.write('\nStorage:\n');
    io.write(
      '  mount <bucket> <path>                         Mount a bucket\n'
    );
    io.write(
      '  umount <path>                                 Unmount a path\n'
    );
    io.write('  df                                            List mounts\n');
    io.write(
      '  flush [path]                                  Flush changes to Tigris\n'
    );
    io.write(
      '  presign <path> [--expires N] [--put] [--key]  Generate a presigned URL\n'
    );
    io.write(
      '  snapshot [<bucket>] [--name N] [--list]       Create or list snapshots\n'
    );
    ...
    io.write(
      'Supported commands: https://github.com/vercel-labs/just-bash#supported-commands\n'
    );
  }
