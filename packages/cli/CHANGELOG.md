...
- [#303](https://github.com/tigrisdata/storage/pull/303) [`60baf07`](https://github.com/tigrisdata/storage/commit/60baf07fbc0b3b0996da8da584c5fcd62c1bc242) Thanks [@claude](https://github.com/apps/claude)! - Fix `tigris --help` on narrow terminals. Commander only wraps a description when the aligned description column is at least 40 characters wide, and our widest command term is 30 characters — so below ~75 columns it stopped wrapping entirely and emitted lines up to 192 characters. Help output now stacks each description under its term and wraps to the terminal width whenever the aligned column would be too narrow to read; 80-column and wider output is unchanged.

## 3.12.0

### Minor Changes

- [#286](https://github.com/tigrisdata/storage/pull/286) [`73d4a3e`](https://github.com/tigrisdata/storage/commit/73d4a3e679845ea1f0f690a0753c4c00400dcbf9) Thanks [@designcode](https://github.com/designcode)! - Add `@tigrisdata/cli/browser`: the CLI running in a page.
  ...
  const { stdout, exitCode } = await createBrowserCli(host).run([
    "objects",
    "list",
    "my-bucket",
  ]);
